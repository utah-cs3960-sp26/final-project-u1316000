# Final Result
Over the last week I made a ton of progress, and got it to a functioning state where I truly can leave it running for long periods of time and come back to a coherent and (in my opinion) interesting story! The first thing that helped me a lot had nothing to do with the project itself. Gemma 4 released, a tiny local model that can run on my laptop with no problems, but it's much more intelligent than any other models of its size. This already helped a ton. Apart from that, I majorly overhauled how the AI generates new nodes. Rather than one-shotting a schema, I now have it output plain text in a "google form" sort of fashion, in steps. This allowed me to give it validation errors / tell it how to fix its output step by step, and let it retry until it gets it right. This made a huge difference in success rate and output quality, and allowed me to create "transition nodes" which serve as a bridge between 2 nodes when doing a merge or a new "self merge" which allows a node to have a choice that loops back to the same spot. This new format also allowed for more dynamic nodes, where the AI can specify when characters show and hide much more easily or use automatic settings such as "show while speaking". I also added a ton of output sanitatization and other allowed formats to make AI output as frictionless as possible. In terms of the database console, I overhauled the look, added cards for both characters and locations, and added an interactive story graph. This was useful as it allowed me to see the story visually and instantly jump to wherever I wanted to observe how the story is evolving.  

Video:
https://www.youtube.com/watch?v=FFbE3GT9EPE

## Week 13
The project has made good progress, but the main thing I've been working on is ensuring the story stays coherent. Last week when I got a head start on this assignment I exclusively used high end models such as chatgpt or gemini to generate the nodes as I went, but as I scale up the project, that stops being a viable option. So I made the swictch to local models (and reset the story) and have had some sub-par results. These smaller models are really good at not being original, and instead just continuing what exists already, which can lead to long chains of actions that are essentially the same thing such as "explore the cave deeper" --> "open the door in the cave" --> "explore deeper into the cave". These models also need much more help with outputting the schemas to translate into the database, and struggle to pass validation sometimes. This week adds a lot of changes to both the validator, and the packet they receive. The system now enforces many things such as story pressure every several turns to tell the AI to make certain bolder decisions, rather than a simple suggestion for conflict or story development. The validator now rejects more types of bad output, such as the before-mentioned chained actions. It is surprisingly simple to stop it from doing this, we just have to check if too many of the words are the same since each new response is so similar to the last that that works. Another huge addition is the ability for the model to "rety" after a rejected story schema. Since it isn't starting fresh, it can actually fix its mistake and validate it's change. There is still a lot of work to do but all the focus on testing and validating the story quality this week as it iterates has helped a ton. 

The smartest thing the AI did this week was help set up a new branch merging system, which greatly helped reduce the exploding choice possibilities without making the story seem restrictive to just a couple paths. It was also very impressive when it one-shotted a live story graph where I can see and pan around to each node and see what choices (on the edges) lead there. The stupidest thing I saw was my overnight run which led me to the "chaining" conclusion from above. I ran the local model in a loop overnight, and despite all the context, story hooks, and worldbuilding I was feeding it, almost every choice it presented was just a repetition of previous choices. Basically 6 hours worth of "follow the path". I was worried there would be no way around this but thankfully this kind of blunder has been fixed now with the new schema validation tests. 
